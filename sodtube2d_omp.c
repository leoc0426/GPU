...
void Init()
{
	...
	for (i = 0; i < NX; i++)
	{
		for (j = 0; j < NY; j++)
		{
			...
			U[i + j*NX][1] = dens[i + j*NX] * (xv[i + j*NX] + yv[i + j*NX]);
			U[i + j*NX][2] = dens[i + j*NX] * (CV*temp[i + j*NX]
...
#pragma omp for collapse(2)// private(speed)
	for (i = 1; i < (NX - 1); i++)
	{
		for (j = 1; j < (NY - 1); j++)
		{
			...
			FL[i + j*NX][0] = 0.5*(dens[i + j*NX - 1] * xv[i + j*NX - 1] + dens[i + j*NX] * xv[i + j*NX]) 
				- speed*(U[i + j*NX][0] - U[i + j*NX - 1][0]);
			FR[i + j*NX][0] = 0.5*(dens[i + j*NX] * xv[i + j*NX] + dens[i + j*NX + 1] * xv[i + j*NX + 1]) 
				- speed*(U[i + j*NX + 1][0] - U[i + j*NX][0]);
			FL[i + j*NX][1] = 0.5*(dens[i + j*NX - 1] * (xv[i + j*NX - 1] * xv[i + j*NX - 1] + R*temp[i + j*NX - 1])
				+ dens[i + j*NX] * (xv[i + j*NX] * xv[i + j*NX] + R*temp[i + j*NX])) 
				- speed*(U[i + j*NX][1] - U[i + j*NX - 1][1]);
			FR[i + j*NX][1] = 0.5*(dens[i + j*NX] * (xv[i + j*NX] * xv[i + j*NX] + R*temp[i + j*NX])
				+ dens[i + j*NX + 1] * (xv[i + j*NX + 1] * xv[i + j*NX + 1] + R*temp[i + j*NX + 1])) 
			...
			FD[i + j*NX][0] = 0.5*(dens[i + (j - 1)*NX] * yv[i + (j - 1)*NX] 
				+ dens[i + j*NX] * yv[i + j*NX]) 
				- speed*(U[i + j*NX][0] - U[i + (j - 1)*NX][0]);
			FU[i + j*NX][0] = 0.5*(dens[i + j*NX] * yv[i + j*NX] 
				+ dens[i + (j + 1)*NX] * yv[i + (j + 1)*NX]) 
				- speed*(U[i + (j + 1)*NX][0] - U[i + j*NX][0]);
			FD[i + j*NX][1] = 0.5*(dens[i + (j - 1)*NX] * (yv[i + (j - 1)*NX] * yv[i + (j - 1)*NX] + R*temp[i + (j - 1)*NX])	
				+ dens[i + j*NX] * (yv[i + j*NX] * yv[i + j*NX] + R*temp[i + j*NX])) 
				- speed*(U[i + j*NX][1] - U[i + (j - 1)*NX][1]);
			FU[i + j*NX][1] = 0.5*(dens[i + j*NX] * (yv[i + j*NX] * yv[i + j*NX] + R*temp[i + j*NX]) 
				+ dens[i + (j + 1)*NX] * (yv[i + (j + 1)*NX] * yv[i + (j + 1)*NX] + R*temp[i + (j + 1)*NX])) 
				- speed*(U[i + (j + 1)*NX][1] - U[i + j*NX][1]);
			FD[i + j*NX][2] = 0.5*(yv[i + (j - 1)*NX] * (U[i + (j - 1)*NX][2] + 	dens[i + (j - 1)*NX] * R*temp[i + (j - 1)*NX])
				+ yv[i + j*NX] * (U[i + j*NX][2] + dens[i + j*NX] * R*temp[i + j*NX])) 
				- speed*(U[i + j*NX][2] - U[i + (j - 1)*NX][2]);
			FU[i + j*NX][2] = 0.5*(yv[i + j*NX] * (U[i + j*NX][2] + dens[i + j*NX] * R*temp[i + j*NX])
