...
#define R (287.0)           // Dimensionless specific gas constant
...
void CPUHeatContactFunction() {
		...
		for (j = 0; j < NY; j++) {
			for (i = 0; i < NX; i++) {
				...
				if (h_body[i + j*NX + (k - 1)*NX*NY] == 0.0) {
					int z;
					for (z = 0; z < 5; z++) {
						printf("xv[%d + %d*NX + %d*NX*NY] = %f\n", i, j, k, xv[i + j*NX + k*NX*NY]);
	...
	float speed = (float)((DX) / DT / 3.0)*0.5;
	...
	for (k = 1; k < (NZ - 1); k++) {
		for (j = 1; j < (NY - 1); j++) {
			for (i = 1; i < (NX - 1); i++) {
				if (h_body[i + j*NX + k*NX*NY] != 0.0) {
					// air
					 //sqrt(GAMA*press[i + j*NX + k*NX*NY] / dens[i + j*NX + k*NX*NY]);		// speed of sound in air
					for (z = 0; z < 5; z++) {
...
//#pragma omp barrier
	...
	//Renew front and back boundary condition
//#pragma omp for
	for (i = 1; i < (NX - 1); i++) {
		for (k = 1; k < (NZ - 1); k++) {
			for (z = 0; z < 5; z++) {
				// back = back+100: 10001 = 10101
				U_new[i + k*NX*NY + z*N] = U_new[i + NX + k*NX*NY + z*N];
				// front = front-100: 19901 = 19801
...
//#pragma omp for
	for (j = 0; j < NY; j++) {
		for (k = 1; k < (NZ - 1); k++) {
			for (z = 0; z < 5; z++) {
				// left = left+1: 10000 = 10001
				U_new[j*NX + k*NX*NY + z*N] = U_new[1+ j*NX + k*NX*NY + z*N];
				
				// right = right-1: 10099 = 10098
		...
		for (j = 0; j < NY; j++) {
			for (i = 0; i < NX; i++) {
				if (h_body[i + j*NX + k*NX*NY] != 0.0) {
					...
					press[i + j*NX + k*NX*NY] = (GAMA - 1) * (U_new[i + j*NX + k*NX*NY + 4 * N] - 0.5 * dens[i + j*NX + k*NX*NY] * (xv[i + j*NX + k*NX*NY] * xv[i + j*NX + k*NX*NY] + yv[i + j*NX + k*NX*NY] * yv[i + j*NX + k*NX*NY] + zv[i + j*NX + k*NX*NY] * zv[i + j*NX + k*NX*NY]));
